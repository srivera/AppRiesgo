...
public class ObtenerContactosAsyncTask extends AsyncTask<String, Float, String>  {

	private ChatBlueToothActivity chatBlueToothActivity;
	private  Context context;

	public ObtenerContactosAsyncTask(Context context,  ChatBlueToothActivity chatBlueToothActivity) {
		super();
	...
	protected String doInBackground(String... arg0) {
		...
		HttpPost httppost = new HttpPost(Constantes.URL_OBTENER_CONTACTOS);
		...
		try {
			...
			String json =  "{\"obtenerContactos\":{\"idPersona\":\""+ datosAplicacion.getCuenta().getId()
			...
			if(statusFlag != null && statusFlag){
				...
				try {
					...
					if(contactos.length() > 0) {
						...
						//contactoDataSource.deleteContactoByTipo("P");
						for (int i = 0, size = contactos.length(); i < size; i++) {
							...
							if(contactoBusqueda == null || contactoBusqueda.getId() == null) {
								...
								contactoBusqueda.setTipo("P");
								contactoDataSource.updateContacto(contactoBusqueda);
							}
